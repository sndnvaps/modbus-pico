...
public:
    ...
    inline void slave(uint16_t port = 0) { server(port); }
...
template <class SERVER, class CLIENT>
void ModbusTCPTemplate<SERVER, CLIENT>::setMode(mode_type mode)
{
    _mode = mode;
}
...
void ModbusTCPTemplate<SERVER, CLIENT>::task()
{
    ...
    if (_mode == MODE_UDP && udpStarted)
    {
        ...
        if (len >= 7)
        {
            ...
            // 读取 MBAP 头
            udp.read(_MBAP.raw, 7);
            _len = __swap_16(_MBAP.length) - 1;

            if (_len > MODBUSIP_MAXFRAME || _len <= 0)
            {
                udp.flush();
                return;
            }
            ...
            if (_reply != REPLY_OFF)
            {
                _MBAP.length = __swap_16(_len + 1);
                uint8_t sbuf[7 + _len];
                memcpy(sbuf, _MBAP.raw, 7);
                memcpy(sbuf + 7, _frame, _len);
                udp.beginPacket(remoteIp, remotePort);
                udp.write(sbuf, 7 + _len);
                udp.endPacket();
            }
    ...
    for (n = 0; n < MODBUSIP_MAX_CLIENTS; n++)
    {
        ...
        {
            tcpclient[n]->readBytes(_MBAP.raw, sizeof(_MBAP.raw));

            if (__swap_16(_MBAP.protocolId) != 0)
            {
            ...
            else
            {
                ...
                {
                    if (tcpclient[n]->readBytes(_frame, _len) < _len)
                    {
                        ...
                        else
                        {
                            ...
                            if (trans)
                            {
                                ...
                                if (it != _trans.end())
                                    _trans.erase(it);
#else
                                size_t r = _trans.find([trans](TTransaction &t)
                                                       { return *trans == t; });
                                _trans.remove(r);
#endif
                            }
